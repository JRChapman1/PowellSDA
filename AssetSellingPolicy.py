...
class AssetSellingPolicy():
    ...
    def __init__(self, model, policy_names):
        ...
        # Set an instance of our asset selling model as a property
        self.model = model

        # Add policy names as a property and use these to create a 'Policy' object
    ...
    def track_policy(self, state, info_tuple):
        ...
        :return: a decision made based on the policy
        """
        # Read policy parameter values
        track_signal = info_tuple[0]
        alpha = info_tuple[1]
        prev_price = info_tuple[2]

        # Smooth the asset price process using specified alpha parameter
        smoothed_price = (1-alpha) * prev_price + alpha * state.price

        # Sell iff current price is greater than the smoothed price plus the track signal
    ...
    def run_policy(self, param_list, policy_info, policy, time):
        ...
        # Create a copy of the model parameter value of the current object
        model_copy = copy(self.model)

        while model_copy.state.resource != 0 and time < model_copy.initial_args['T']:

            # Create an instance of the 'Policy' object, which effectively just holds parameter values for each possible
            # policy types
            ...
            # sell regardless of policy if terminal time has been reached.
            if (time == model_copy.initial_args['T'] - 1):
                 decision = {'sell': 1, 'hold': 0}  

            # Parse decision from dictionary to decision object
        ...
        # Return the contribution of the policy run
